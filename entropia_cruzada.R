...
for (i in 1:36760){
   ...
   fone<-amostra[[i]]$prob_phoneme$word # pega palavra fonetica de referencia
   ...
   phone<-numeric() # palavra transcrita
  
   for (ind2 in 1:num_fones) phone[ind2]<-tabela[[ind2]][1,1] # pega os fones da transcricao do modelo
   
   if (grepl(f,fone)){ # verifica se o fone esta na palavra fonetica de referencia
     
     count_palavras_com_fone[[ind]][i]<-1 # atualiza vetor de contagem de palavras com o fone
   }
